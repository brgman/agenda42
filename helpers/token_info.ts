...
export default async function getVerifyTokenInfo(req: any, res: any) {
    ...
    try {
        ...
        if ((req.method === "POST" || req.method === "DELETE") && req.body["user_id"].toString() !== isAuth.data?.resource_owner_id)
            return res.status(401).json({ status: "Forbidden" });

        if ((req.method === "GET" || req.method === "PATH") && req.query["user_id"].toString() !== isAuth.data?.resource_owner_id)
            return res.status(401).json({ status: "Forbidden" });
    } catch (e) {
        return res.status(403).json({ status: "Forbidden" });
    }
